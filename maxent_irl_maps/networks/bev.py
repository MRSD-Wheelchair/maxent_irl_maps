...
class BEVToCostSpeed(nn.Module):
    ...
    def run_cost_speed_heads(self, features, return_mean_entropy):
        ...
        if self.cost_type == "Categorical":
            ...
            if return_mean_entropy:
                res["costmap"], res["costmap_entropy"] = compute_map_mean_entropy(
                    cost_logits, self.cost_bins
                )
        ...
        if self.speed_type == "Categorical":
            ...
            if return_mean_entropy:
                res["speedmap"], res["speedmap_entropy"] = compute_map_mean_entropy(
                    speed_logits, self.speed_bins
                )
    ...
    def normalize_bev_features(self, x):
        ...
        return ((x - _mean) / _std).clip(-10.0, 10.0)
    ...
    def setup_cost_head(self, params):
        ...
        if params["type"] == "Categorical":
            ...
            self.cost_bins = torch.linspace(
                self.min_cost, self.max_cost, self.cost_nbins + 1, device=self.device
            )
    ...
    def setup_speed_head(self, params):
        ...
        if params["type"] == "Categorical":
            ...
            self.speed_bins = torch.linspace(
                self.min_speed, self.max_speed, self.speed_nbins + 1, device=self.device
            )
...
class VoxelRecolorBEVToCostSpeed(BEVToCostSpeed):
    ...
    def forward(self, x, return_mean_entropy=True):
        ...
        splat_voxel_features = self.terrain_splat_voxel_features(
            voxel_recolor_data, terrain
        )

        bev_feat_idxs = [
            x["bev_data"]["feature_keys"].index(k) for k in self.bev_layers
        ]
    ...
    def terrain_splat_voxel_features(self, voxel_data, terrain):
        ...
        _feat_mask = (
            _voxels.features[:, voxel_data["feature_keys"].index("feature_mask")] > 0.5
        )
...
class BEVLSSToCostSpeed(BEVToCostSpeed):
    ...
    def forward(self, x, return_mean_entropy=True):
        ...
        max_depth = torch.linalg.norm(x["bev_data"]["metadata"].length[0]) / 2.0
        with torch.no_grad():
        ...
        _metadata = torch.stack(
            [bev_metadata.origin, bev_metadata.length, bev_metadata.resolution], dim=1
        )
    ...
    def sample_camera_frustums(self, pose, image, intrinsics, n_bins, max_depth):
        ...
        depth_bins = torch.linspace(0.0, max_depth, n_bins + 1, device=self.device)[1:]
    ...
    def transform_frustums(self, frustums, poses):
        ...
        _pts = torch.cat(
            [frustums, torch.ones_like(frustums[..., [0]])], dim=-1
        ).unsqueeze(
            -1
        )  # [BxWxHxDx4x1]
    ...
    def setup_lss(self, params):
        self.lss = LSS(image_insize=self.image_insize, device=self.device, **params)
